...
# MAGIC
# MAGIC All the logic is in `retailpulse/medallion/bronze.py` -- the same module
...
# A Git folder puts only the *repo root* on sys.path (DBR 11.3+), and the
# package lives one level down in project_2/ -- this notebook's parent.
...
# config.py falls back to its defaults -- catalog `workspace`, schema
